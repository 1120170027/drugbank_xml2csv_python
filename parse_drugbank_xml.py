...
template2attrib = {'targets': defaultdict(dict), 'enzymes': defaultdict(
    dict), 'transporters': defaultdict(dict), 'carriers': defaultdict(dict)}

sin = {"targets": "target", "enzymes": "enzyme",
...
for child in root:
    ...
    groups = [s.text for s in child.find(
        tag_prefix + 'groups').findall(tag_prefix + 'group')]
    ...
    for template in ['targets', 'enzymes', 'transporters', 'carriers']:
        ...
        for res in child.find(tag_prefix + template).findall(tag_prefix + sin[template]):
            if res.find(tag_prefix + 'polypeptide') is None:
                continue
            template_id = res.find(tag_prefix + 'id').text
            template_gene = res.find(
                tag_prefix + 'polypeptide').find(tag_prefix + 'gene-name').text
            template_name = res.find(tag_prefix + 'name').text
            template_organism = res.find(tag_prefix + 'organism').text
            template_taxonomy_id = res.find(
                tag_prefix + 'polypeptide').find(tag_prefix + 'organism').attrib['ncbi-taxonomy-id']
            ...
            if template_organism == 'Homo sapiens':
                template_organism = 'Human'
            if template_gene is None or template_organism is None or template_taxonomy_id is None:
                continue

            template_external_ids = res.find(tag_prefix + 'polypeptide').find(
                tag_prefix + 'external-identifiers').findall(tag_prefix + 'external-identifier')
            template_uniprot_id = ''
            ...
            template_actions = [s.text.lower() for s in res.find(
                tag_prefix + 'actions').findall(tag_prefix + 'action')]

            drug2attrib[drugbank_id][template].append(
                (template_id, template_actions))
            ...
            # print template_id, template_gene, template_name,
            # template_organism, template_taxonomy_id, template_actions

    # print drugbank_id, drugname, drug_type, groups,
    # print 'targets:', len(template2attrib['targets'][drugbank_id]['targets']),
    # print 'enzymes:', len(template2attrib['enzymes'][drugbank_id]['enzymes']),
    # print 'transporters:', len(template2attrib['transporters'][drugbank_id]['transporters'])
    # print 'carriers:',
    # len(template2attrib['carriers'][drugbank_id]['carriers'])
...
print len(drugs), "drugs with at least 1 target/ enzyme/ transporter / carriers"
...
writer.writerow(['drugbank_id', 'drugname', 'drug_type', 'approved',
...
writer.writerow(['partner_id', 'partner_name', 'gene_name', 'uniprot_id',
                 'genbank_gene_id', 'genbank_protein_id', 'hgnc_id', 'organism', 'taxonomy_id'])
writerh.writerow(['partner_id', 'partner_name', 'gene_name', 'uniprot_id',
                  'genbank_gene_id', 'genbank_protein_id', 'hgnc_id', 'organism', 'taxonomy_id'])
...
for template in ['targets', 'enzymes', 'transporters', 'carriers']:
    for partner_id in sorted(template2attrib[template].keys()):
        ...
        genbank_gene_id = template2attrib[
            template][partner_id]['genbank_gene_id']
        genbank_protein_id = template2attrib[
            template][partner_id]['genbank_protein_id']
        ...
        if taxonomy_id == '9606' and organism.lower() != 'human':
            print partner_id, template2attrib[template][partner_id]['gene'], organism, taxonomy_id, 'organism mismatch'
